...
def signup(request):
    if request.method=="POST":   
        ...
        applicants.save()
        thank1 = True
        return render(request, "signup.html", {"thank1":thank1})
...
def company_signup(request):
    if request.method=="POST":   
        ...
        company.save()
        thank2 = True
        return render(request, "company_signup.html", {"thank2":thank2})
...
def company_login(request):
    if request.method == "POST":
        ...
        if user is not None:
            ...
            elif user1.type == "company" and (user1.status == "pending" or user1.status == "rejected"):
                alert1 = True
                return render(request, "company_login.html", {"alert1":alert1})
        else:
            alert = True
